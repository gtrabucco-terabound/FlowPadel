...
begin
  ...
  -- Ya confirmada: solo marcar seña pagada.
  ...
  -- Asegurar jugador 1 (dedup por teléfono; crear si no existe).
  v_p1 := r.player_1_id;
  ...
  -- Asegurar jugador 2 (si hay pareja).
  v_p2 := r.player_2_id;
  ...
  if not exists (select 1 from payments where registration_id=p_registration_id and kind='inscription') then
    ...
    values (e.club_id, r.event_id, p_registration_id, 'inscription',
      ...
      case when coalesce(e.inscription_per_person,0)=0 then 'paid' else 'pending' end, 'cash',
      case when coalesce(e.inscription_per_person,0)=0 then now() else null end);
  end if;
  ...
  if e.charge_court and not exists (select 1 from payments where registration_id=p_registration_id and kind='court_fee') then
    ...
    values (e.club_id, r.event_id, p_registration_id, 'court_fee',
      ...
      case when coalesce(e.court_fee_per_person,0)=0 then 'paid' else 'pending' end, 'cash',
      case when coalesce(e.court_fee_per_person,0)=0 then now() else null end);
  end if;
end $$;
