-- Reserva pública consciente de las franjas de turno (court_slot_bands):
-- el precio y la duración del turno salen de la franja donde cae el inicio.
...
begin
  ...
  -- Duración y precio según la franja que contiene el inicio (o el default de la cancha).
  select coalesce(sb.slot_minutes, c.slot_minutes), coalesce(sb.price, c.price_per_slot)
    into v_slot, v_price
    from courts c
    left join court_slot_bands sb
      on sb.court_id = c.id and p_start >= sb.start_minutes and p_start < sb.end_minutes
   where c.id=p_court_id and c.club_id=v_club and c.is_active
   limit 1;
  ...
  select jsonb_build_object(
    ...
    'courts', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'operating_days',co.operating_days,
        'bands', coalesce((
          select jsonb_agg(jsonb_build_object(
            'start_minutes',sb.start_minutes,'end_minutes',sb.end_minutes,
            'slot_minutes',sb.slot_minutes,'price',sb.price) order by sb.start_minutes)
          from court_slot_bands sb where sb.court_id=co.id
        ),'[]'::jsonb)
      ) order by co.number)
      from courts co where co.club_id=(select id from cl) and co.is_active
        and (coalesce(co.price_per_slot,0) > 0
             or exists (select 1 from court_slot_bands sb where sb.court_id=co.id))
    ),'[]'::jsonb),
    'bookings', coalesce((
