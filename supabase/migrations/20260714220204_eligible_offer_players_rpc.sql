-- Devuelve jugadores elegibles para una invitación dirigida de oferta.
-- Respeta consentimiento (receive_offers + notify_whatsapp + phone), del club,
-- que sean "fieles" (>= min_matches partidos) o "dormidos" (última reserva > inactive_days),
-- y que NO hayan recibido invitación hoy. Ordena fieles primero, limita a max_n.
...
as $$
  with last_book as (
    ...
    where club_id = p_club_id and player_id is not null
      and status in ('confirmed','paid','held')
    group by player_id
  )
